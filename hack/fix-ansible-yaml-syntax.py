...
def fix_yaml_indentation(file_path):
    ...
    module_re = re.compile(r'^(?P<indent>\s*)(ansible\.builtin\.|ansible\.legacy\.|kubernetes\.core\.|amazon\.aws\.|community\.[a-zA-Z_]+\.)\w+:')
    task_keys = ("register", "when", "changed_when", "failed_when", "ignore_errors", "loop", "retries", "delay", "until")
    ...
    while i < len(lines):
        ...
        if last_task_indent is not None and any(line.lstrip().startswith(k + ":") for k in task_keys):
