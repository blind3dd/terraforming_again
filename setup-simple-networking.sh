...
# Simplified Volume Networking Setup
# Creates network configurations for provider-grouped volumes without physical interfaces
...
show_cross_provider_info() {
    ...
    for other_provider_info in "${PROVIDER_NETWORKS[@]}"; do
        ...
        if [[ "$other_provider" != "$PROVIDER" ]]; then
            echo "  $other_provider provider: $other_network (gateway: $other_gateway)"
        fi
    done
}
...
# Show detailed info
/opt/nix-volumes/networking/manage-volume-networking.sh info

# Show cross-provider connectivity
/opt/nix-volumes/networking/manage-volume-networking.sh cross-provider
```
EOF
...
main() {
    ...
    log ""
    log "Next steps:"
    log "1. Check status: $NETWORK_DIR/manage-volume-networking.sh status"
    log "2. Test configurations: $NETWORK_DIR/manage-volume-networking.sh test"
    log "3. Show detailed info: $NETWORK_DIR/manage-volume-networking.sh info"
    log "4. View architecture: cat $NETWORK_DIR/volume-networking-architecture.md"
}
