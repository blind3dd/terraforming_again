...
# Setup script using pipenv for dependency management
# This script installs pipenv and sets up the Python environment
...
install_pipenv() {
    ...
    if command -v pipenv &> /dev/null; then
        log_success "pipenv is already installed: $(pipenv --version)"
        return
    fi
    
    if command -v pip3 &> /dev/null; then
        pip3 install pipenv
        log_success "pipenv installed via pip3"
...
install_dependencies() {
    log_info "Installing dependencies using pipenv..."
...
test_inventory() {
    ...
    # Test --list using pipenv
    if pipenv run python ec2.py --list &> /dev/null; then
    ...
    pipenv run python ec2.py --list --pretty 2>/dev/null || echo "No instances found"
}
...
show_usage() {
    ...
    log_info ""
    log_info "  # Activate the virtual environment"
    log_info "  pipenv shell"
    log_info ""
    log_info "  # Run commands in the virtual environment"
    log_info "  pipenv run ansible webservers -m ping"
    log_info "  pipenv run ansible go_mysql_api_instances -m shell -a 'whoami'"
    log_info "  pipenv run ansible-playbook database-init.yml"
    log_info ""
    log_info "  # Use convenience scripts"
    log_info "  pipenv run inventory-list"
    log_info "  pipenv run ansible-ping"
    log_info ""
    log_info "  # Development tasks"
    log_info "  pipenv run test"
    log_info "  pipenv run format"
    log_info "  pipenv run lint"
    log_info "  pipenv run type-check"
    log_info ""
    ...
    log_info ""
    log_info "📁 Virtual environment location:"
    log_info "   $(pipenv --venv)"
}
